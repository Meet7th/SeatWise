...
info "  用途: 自动创建数据库项目"
info "  获取步骤:"
info "    1. 打开 ${CYAN}https://supabase.com${NC}，用 GitHub 登录"
info "    2. 点击左下角头像 → Account Settings"
info "    3. 点击左侧 ${CYAN}Access Tokens${NC}"
info "    4. 点击 ${CYAN}Generate new token${NC}"
info "    5. 输入名称如 ${CYAN}seatwise-deploy${NC}"
info "    6. 点击 Generate token，复制 Token"
echo ""
info "  ${BOLD}Token 2: Vercel Access Token${NC}"
info "  用途: 自动部署前端和后端"
info "  获取步骤:"
info "    1. 打开 ${CYAN}https://vercel.com${NC}，用 GitHub 登录"
info "    2. 点击右上角头像 → Settings"
info "    3. 点击左侧 ${CYAN}Tokens${NC}"
info "    4. 点击 ${CYAN}Create${NC}"
info "    5. 输入名称如 ${CYAN}seatwise-deploy${NC}"
info "    6. 点击 Create Token，复制 Token"
echo ""
info "  ${BOLD}Token 3: GitHub Personal Access Token${NC}"
info "  用途: 自动 Fork 仓库到你的账号"
info "  获取步骤:"
info "    1. 打开 ${CYAN}https://github.com${NC}，登录账号"
info "    2. 点击右上角头像 → Settings"
info "    3. 滚动到底部 → Developer settings"
info "    4. 点击 ${CYAN}Personal access tokens → Fine-grained tokens${NC}"
info "    5. 点击 ${CYAN}Generate new token${NC}"
info "    6. 输入名称如 ${CYAN}seatwise-deploy${NC}"
info "    7. Repository access 选择 ${CYAN}All repositories${NC}"
info "    8. Permissions 找到 ${CYAN}Contents${NC}，选择 ${CYAN}Read and write${NC}"
info "    9. 点击 Generate token，复制 Token"
echo ""
info "  ${YELLOW}⚠ Token 只会显示一次，请立即复制保存！${NC}"
